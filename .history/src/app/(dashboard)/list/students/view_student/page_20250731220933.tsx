...
import { useState, useMemoوuseEffect } from "react"
...
export default function StudentsDataPage() {
 ...
 useEffect(() => {
    const fetchStudents = async () => {
      setIsLoading(true);
      // سنقوم بإنشاء API route لجلب البيانات
      const response = await fetch('/api/students'); // نفترض أن هذا هو مسار الـ API
      const data = await response.json();
      
      // تنسيق البيانات هنا بعد جلبها
      const formattedStudents = data.map((student: any) => ({
        ...
        stage: student.grade.level.toString(),
    ...
    fetchStudents();
  }, []); // [] تعني أن هذا التأثير يعمل مرة و
